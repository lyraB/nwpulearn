...
import com.nwpu.content.mapper.CoursePublishMapper;
import com.nwpu.content.mapper.CoursePublishPreMapper;
...
import com.nwpu.messagesdk.model.po.MqMessage;

import com.nwpu.messagesdk.service.MqMessageService;
...
public class CoursePublishServiceImpl implements CoursePublishService {
    ...
    public CoursePreviewDto getCoursePreviewInfo(Long courseId) {
        ...
        if(courseBaseInfo.getStatus().equals("203001") || courseBaseInfo.getStatus().equals("203003")){
            NwpuException.cast("课程未发布或已下线");
        }
        //课程计划信息
    ...
    public void commitAudit(Long companyId, Long courseId) {
        ...
        if("202003".equals(auditStatus)){
            NwpuException.cast("当前为等待审核状态，审核完成可以再次提交。");
        }
        //本机构只允许提交本机构的课程
        if(!courseBase.getCompanyId().equals(companyId)){
            NwpuException.cast("不允许提交其它机构的课程。");
        }

        //课程图片是否填写
        if(StringUtils.isEmpty(courseBase.getPic())){
            NwpuException.cast("提交失败，请上传课程图片");
        }
        ...
        if(teachplanTree.size()<=0){
            NwpuException.cast("提交失败，还没有添加课程计划");
        }
        //转json
    ...
    public void publish(Long companyId, Long courseId) {
        ...
        if(coursePublishPre == null){
            NwpuException.cast("请先提交课程审核，审核通过才可以发布");
        }
        //本机构只允许提交本机构的课程
        ...
        if(!"202004".equals(auditStatus)){
            NwpuException.cast("操作失败，课程审核通过方可发布。");
        }
...
//            System.out.println(content);
    ...
    private void saveCoursePublishMessage(Long courseId){
        ...
        if(mqMessage==null){
            NwpuException.cast(CommonError.UNKOWN_ERROR);
        }
